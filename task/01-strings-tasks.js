...
function getRectangleString(width, height) {
    ...
    let rectangleString = top + medium + bottom;
...
function encodeToRot13(str) {
    return str.split('').map(function(item) {
        let indexCode       = item.codePointAt();
        let checkFirstHalf  = (indexCode >= 65 && indexCode < 78) || (indexCode >= 97 && indexCode < 110);
        let checkLastHalf   = (indexCode >= 78 && indexCode <= 90) || (indexCode >= 110 && indexCode <= 122);

        if (checkFirstHalf) {
            item = String.fromCodePoint(indexCode + 13);
        } else if (checkLastHalf) {
            item = String.fromCodePoint(indexCode - 13);
        }
...
function isString(value) {
    let boolean = typeof value == 'string';

    if (value === null && typeof value != 'undefined') {
        boolean = false;
    } else if (typeof value == 'object') {
        boolean = typeof value.valueOf() == 'string';
    }
